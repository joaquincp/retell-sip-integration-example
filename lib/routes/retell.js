...
const service = ({logger, makeService}) => {
  ...
  svc.on('session:new', async(session) => {
    ...
    /* Keep alive because of heroku 55s timeout on https an wss */
    keepAlives[session.call_sid] = setInterval(() => {
      session.send('ping');
      /*session.sip_request({
        method: 'INFO',
        headers: { 
          "X-Metadata": "keep alive" 
        },
        actionHook: '/info'
      });*/
    }, 10000);
    ...
    session
      ...
      .on('/info', onInfo.bind(null, session));
...
const onClose = (session, code, reason) => {
  const {logger} = session.locals;
  clearInterval(keepAlives[session.call_sid]);
...
const onInfo = (session, code, reason) => {
  const {logger} = session.locals;
  logger.info({session, code, reason}, `session ${session.call_sid} info`);
  session.reply();
};
